...
```{r}
comparison_stats_all <- compare_means(sum ~ location, 
                                      data=count_phos_sum, group.by="Genus")

my_comparisons <- list(c("ras","control"),
                       c("ras","test"),
                       c("control","test"))

ggplot(data=count_phos_sum,aes(x=location,y=sum,color=location,shape=location)) +
  facet_wrap(~Genus, scales="free", ncol=4, labeller=labels_comp) +
  geom_boxplot(outlier.shape=NA) + 
  geom_point(alpha=0.5) +
  theme_classic() +
...
ggsave(outpath_figures %>% file.path("mean_count_compare_pao.tiff"),
       width=9, height=5, units="in", dpi=300)
```
...
Test basin only by phase
```{r}
rel_phos_sum %>% filter(location == "test") %>% 
  filter(Genus %in% c("Ca_Accumulibacter","Dechloromonas","Tetrasphaera",
                      "Ca_Competibacter", "Defluviicoccus")) %>%
  ggplot(data=.,mapping=aes(x=phase, y=sum, fill=Genus)) +
  facet_wrap(~Genus, labeller=labels_comp, nrow=1) + 
  geom_boxplot(outlier.colour = NA) + 
  geom_point(alpha=0.6) + 
  labs(y="Relative abundance [%]",x="Phase") +
  theme_bw() +
  theme(legend.position="none",strip.text.x = element_text(size = 8)) + 
  scale_fill_paogao_select

ggsave(outpath_figures %>% file.path("pao_gao_test_box.tiff"),
       width=6, height=2.5, units="in", dpi=300)
```
```{r by count}
count_phos_sum %>% filter(location == "test") %>% 
  filter(Genus %in% c("Ca_Accumulibacter","Dechloromonas","Tetrasphaera",
                      "Ca_Competibacter", "Defluviicoccus")) %>%
  ggplot(data=.,mapping=aes(x=phase, y=sum, fill=Genus)) +
  facet_wrap(~Genus, labeller=labels_comp, nrow=1) + 
  geom_boxplot(outlier.colour = NA) + 
  geom_point(alpha=0.6) + 
  labs(y="Read count",x="Phase") +
  theme_bw() +
  theme(legend.position="none",strip.text.x = element_text(size = 8)) + 
  scale_fill_paogao_select

ggsave(outpath_figures %>% file.path("pao_gao_test_box_count.tiff"),
       width=6, height=2.5, units="in", dpi=300)
```

Graphical abstract little guy
```{r}
rel_phos_sum %>% filter(location == "test") %>% 
  filter(Genus %in% c("Dechloromonas", "Ca_Competibacter")) %>%
  ggplot(data=.,mapping=aes(x=phase, y=sum, fill=Genus)) +
  geom_boxplot(outlier.colour = NA, lwd=0.5) + 
  labs(y="% abundance",x="Phase") +
  theme_classic() +
  theme(legend.position="none", 
        axis.ticks=element_blank(), axis.text=element_blank()) + 
  scale_fill_paogao_select

ggsave(outpath_figures %>% file.path("pao_gao_abstract.tiff"),
